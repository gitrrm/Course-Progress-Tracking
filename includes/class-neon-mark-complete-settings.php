...
class Neon_Mark_Complete_Settings {
    ...
    public function settings_init() {
        ...
        add_settings_field(
            'post_page_switch',
            'Posts and Pages Checkbox/Switch',
    ...
    public function sanitize($input) {
        ...
        if (isset($input['enable_posts'])) {
            $new_input['enable_posts'] = $input['enable_posts'] ? '1' : '0';
        }
        if (isset($input['enable_pages'])) {
            $new_input['enable_pages'] = $input['enable_pages'] ? '1' : '0';
        }
        ...
        if (isset($input['post_page_switch'])) {
            $new_input['post_page_switch'] = $input['post_page_switch'] ? '1' : '0';
        }
    ...
    public function show_widget_callback() {
        printf(
            '<input type="checkbox" id="show_widget" name="neon_mark_complete_options[show_widget]" value="1" %s />',
    ...
    public function enable_posts_callback() {
        printf(
            '<input type="checkbox" id="enable_posts" name="neon_mark_complete_options[enable_posts]" value="1" %s />',
            isset($this->options['enable_posts']) && $this->options['enable_posts'] === '1' ? 'checked' : ''
        );
    }

    public function enable_pages_callback() {
        printf(
            '<input type="checkbox" id="enable_pages" name="neon_mark_complete_options[enable_pages]" value="1" %s />',
            isset($this->options['enable_pages']) && $this->options['enable_pages'] === '1' ? 'checked' : ''
        );
    }
    ...
    public function quick_edit_callback() {
        printf(
            '<input type="checkbox" id="quick_edit" name="neon_mark_complete_options[quick_edit]" value="1" %s />',
    ...
    public function post_page_switch_callback() {
        printf(
            '<input type="checkbox" id="post_page_switch" name="neon_mark_complete_options[post_page_switch]" value="1" %s />',
    ...
    public function placement_callback() {
        ...
        foreach ($placements as $value => $label) {
            ...
            printf(
                '<label><input type="radio" name="neon_mark_complete_options[placement]" value="%s" %s> %s</label><br>',
                esc_attr($value),
                $checked,
                esc_html($label)
            );
